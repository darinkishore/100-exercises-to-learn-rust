...
//   We've changed the enum variants to be more specific, thus removing the need
// for storing   a `String` field into each variant.
//   You'll also have to add `thiserror` as a dependency in the `Cargo.toml`
// file.
...
mod tests {
    ...
    fn title_cannot_be_empty() {
        let err = Ticket::new("".into(), valid_description(), Status::ToDo,)
            .unwrap_err();
    ...
    fn description_cannot_be_empty() {
        let err =
            Ticket::new(valid_title(), "".into(), Status::ToDo,).unwrap_err();
