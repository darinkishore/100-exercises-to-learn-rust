...
//  When implementing `Display`, you may want to use the `write!` macro from
// Rust's standard library.  The docs for the `std::fmt` module are a good place
// to start and look for examples:  https://doc.rust-lang.org/std/fmt/index.html#write
...
// TODO: `easy_ticket` should panic when the title is invalid, using the error
// message   stored inside the relevant variant of the `TicketNewError` enum.
//   When the description is invalid, instead, it should use a default
