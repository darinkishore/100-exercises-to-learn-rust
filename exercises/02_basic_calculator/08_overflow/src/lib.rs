...
//
// For reasons that we'll explain later, the customization needs to be done in
// the `Cargo.toml` at the root of the repository, not in the `Cargo.toml` of
// the exercise.
...
mod tests {
    ...
    fn twentieth() {
        ...
        // With the default dev profile, this will panic when you run `cargo
        // test` We want it to wrap around instead
